...
import tg_texte   # gemeinsame Telegram-Textbausteine (auch vom Crypto-Bot genutzt)
...
class SuperTradingBot:
    ...
    def send(self, msg, roh=False):
        """Telegram-Nachricht. HTML ist aktiv — dynamische Texte vorher durch _esc().

        roh=True unterdrueckt die Kopfzeile (fuer Nachrichten, die ihre eigene haben).
        """
    ...
    # ── Aufbereitung fuer Telegram ─────────────────────────────────────────
    #
    # Der Feed soll auf dem Handy in einer Sekunde lesbar sein. Deshalb:
    # deutsche Zahlen, Klartext statt Indikator-Kuerzel, Kernaussage zuerst.
    # Die vollstaendigen Kennzahlen stehen weiterhin in der Konsole und im
    # Dashboard — Telegram ist die Zusammenfassung, nicht das Logbuch.

    # Tabellen und Zahlenformate liegen in tg_texte.py — gemeinsam mit dem
    # Crypto-Bot, damit ein neuer Exit-Grund nicht in einem Bot uebersetzt
    # wird und im anderen als Kuerzel stehen bleibt.
    _esc   = staticmethod(tg_texte.esc)
    _de    = staticmethod(tg_texte.de)
    _vz    = staticmethod(tg_texte.vz)
    _dauer = staticmethod(tg_texte.dauer)

    def _titel(self, symbol):
        """XLK (Technologie) — Sektor auf Deutsch, wenn bekannt."""
        sektor = tg_texte.SEKTOR_DE.get(
            next((s for s, e in ETFS.items() if e == symbol), ""), "")
        return tg_texte.esc(symbol) + (" (" + sektor + ")" if sektor else "")

    def _tg_kauf(self, symbol, shares, price, einsatz, risiko, regime, rsi,
                 score_pct, threshold, ml_prob, size_mult, dd_zone, vix_regime):
        """Kauf-Nachricht. Eigene Methode, damit sie ohne laufenden Bot pruefbar ist."""
        risiko_pct = (risiko / self.balance * 100) if self.balance else 0
        zeilen = [
            "🟢 <b>KAUF · " + self._titel(symbol) + "</b>",
            "",
            # menge() statt fester Rundung: der Bot kauft Bruchstuecke
            # (0,3652 GLD) — auf 0 Stellen stuende dort "0 Stück".
            tg_texte.menge(shares) + " Stück zu " + tg_texte.preis(price) + " $"
            + "  =  " + tg_texte.de(einsatz) + " $",
            "Risiko: " + tg_texte.de(risiko) + " $ (" + tg_texte.de(risiko_pct, 1)
            + " % vom Konto)",
            "",
            "Lage: " + tg_texte.lage(regime) + ", "
            + ("nicht überkauft" if rsi < 65 else "bereits weit gelaufen"),
            "Signal: " + str(round(score_pct * 100)) + " % (nötig wären "
            + str(int(threshold * 100)) + " %)",
        ]
        if ml_prob is not None:
            zeilen.append("Trefferchance laut Modell: " + str(round(ml_prob * 100)) + " %")
        if size_mult < 1.0:
            zeilen.append("Kleiner gekauft als üblich — "
                          + tg_texte.ZONE_DE.get(dd_zone, tg_texte.esc(dd_zone)) + ", "
                          + tg_texte.VIX_DE.get(vix_regime, tg_texte.esc(vix_regime)))
        zeilen += ["", "Kontostand: " + tg_texte.de(self.balance) + " $"]
        return "\n".join(zeilen)

    def _tg_verkauf(self, symbol, profit, pnl_pct, reason, gehalten_seit, bal):
        """Verkaufs-Nachricht — Aufbau kommt aus tg_texte, identisch zum Crypto-Bot."""
        return tg_texte.verkauf(self._titel(symbol), profit, pnl_pct,
                                reason, gehalten_seit, bal)
    ...
    def _check_held_earnings(self):
        ...
        for sym in symbols:
            ...
            if blocked:
                ...
                print("[EARNINGS] " + msg)
                self.send("📅 <b>Zahlen voraus · " + self._titel(sym) + "</b>"
                          "\n\n" + self._esc(stock) + " legt am " + self._esc(ed)
                          + " Quartalszahlen vor — ein großer Wert in diesem Korb."
                          "\n\nDie Position bleibt bestehen. Der Bot verkauft deswegen"
                          " nichts von allein; das entscheidest du.", roh=True)
    ...
    def close_position(self, symbol, price, reason, pnl_pct):
        ...
        # Telegram: auf einen Blick erkennbar, ob Gewinn oder Verlust.
        self.send(self._tg_verkauf(symbol, profit, pnl_pct, reason,
                                   pos.get("time", ""), bal), roh=True)
        ...
        for symbol in symbols:
            try:
                ...
                if reason:
                    ...
                    # Keine eigene Telegram-Meldung mehr: close_position schickt
                    # die Verkaufsnachricht inkl. Grund ("zu lange gehalten").
                    # Vorher kamen zwei Nachrichten fuer denselben Vorgang — und
                    # die hier enthielt <b>-Tags ohne parse_mode, stand also
                    # woertlich im Chat.
    ...
    def _get_drawdown_mult(self):
        ...
        if day_pct > -3.0:
            ...
            if not getattr(self, "_dd_caution_sent", False):
                self.send("⚠️ <b>Vorsicht</b> · " + self._vz(day_pct, 1) + " % heute"
                          "\n\nNeue Positionen werden ab jetzt um 30 % kleiner gekauft."
                          "\nGehaltene Positionen bleiben unberührt.", roh=True)
                self._dd_caution_sent = True
            return 0.7, "CAUTION"
        elif day_pct > -9.0:
            if not getattr(self, "_dd_warning_sent", False):
                self.send("🔴 <b>Warnung</b> · " + self._vz(day_pct, 1) + " % heute"
                          "\n\nNeue Positionen nur noch in 40 % der normalen Größe."
                          "\nGehaltene Positionen bleiben unberührt.", roh=True)
                self._dd_warning_sent = True
            return 0.4, "WARNING"
        else:
            if not getattr(self, "_dd_danger_sent", False):
                self.send("🚨 <b>Gefahrenzone</b> · " + self._vz(day_pct, 1) + " % heute"
    ...
    def trade(self, scores, quelle="Gesamtbild"):
        ...
        for sector, score in ranked:
            ...
            # Telegram bekommt die lesbare Kurzfassung — die Kennzahlen oben
            # bleiben vollstaendig in der Konsole und im Dashboard.
            self.send(self._tg_kauf(
                symbol, shares, price, shares * fill, shares * ind["atr"] * 2,
                regime, ind.get("rsi", 0), score_pct, threshold,
                ml_prob if self._ml_model is not None else None,
                size_mult, dd_zone, vix_regime), roh=True)
    ...
    def check_day_loss(self):
        ...
        if loss >= self.max_day_loss:
            ...
            self.send("🛑 <b>Tageslimit erreicht</b>"
                      "\n\nHeutiger Verlust: <b>" + self._vz(-loss * 100, 1) + " %</b>"
                      "\nGrenze: " + self._de(self.max_day_loss * 100, 0) + " %"
                      "\n\n<b>Der Bot kauft heute nichts mehr.</b>"
                      "\nGehaltene Positionen laufen mit ihren Stops weiter."
                      "\nMorgen früh startet er von selbst wieder.", roh=True)
            self.running = False
    ...
    def run(self, interval=600):
        ...
        self.send("▶️ <b>Super-Bot gestartet</b>"
                  "\n\nKontostand: <b>" + self._de(self.balance) + " $</b>"
                  "\nOffene Positionen: " + str(offen)
        ...
        while True:
            try:
                ...
                self.send("⏹️ <b>Super-Bot gestoppt</b>"
                          "\n\nKontostand: " + self._de(self.balance) + " $"
                          "\nEs wird nicht mehr gehandelt, bis er neu startet.", roh=True)
                break
            except Exception as e:
                print("[ERROR] " + str(e))
                time.sleep(60)
