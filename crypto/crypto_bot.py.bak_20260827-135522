...
# Gemeinsame Telegram-Textbausteine (liegt wie config.py eine Ebene hoeher).
# Denselben Aufbau nutzt super_bot.py — eine Tabelle statt zwei Kopien.
try:
    import tg_texte
except ImportError:
    tg_texte = None
...
class CryptoBot:
    ...
    def send(self, msg, roh=False):
        """roh=True: Nachricht bringt ihre eigene Kopfzeile mit (neuer Stil)."""
    ...
    def _titel(self, symbol):
        """BTC/USD -> 'BTC', Meme-Coins bekommen einen Hinweis."""
        # CRYPTO_MEME ist die Modulkonstante; WILD_MEME existiert nur lokal in trade().
        kurz = tg_texte.esc(str(symbol).split("/")[0]) if tg_texte else str(symbol)
        if symbol in CRYPTO_MEME:
            return kurz + " (Meme-Coin)"
        return kurz

    def _tg_kauf(self, symbol, shares, price, regime, rsi, score_pct,
                 risiko, size_mult, dd_zone, vol_regime, spike_ratio=None):
        """Kauf-Nachricht im gemeinsamen Stil. Eigene Methode = ohne Bot pruefbar."""
        t = tg_texte
        einsatz = shares * price
        risiko_pct = (risiko / self.balance * 100) if self.balance else 0
        blitz = spike_ratio is not None

        zeilen = [
            ("⚡ <b>BLITZ-KAUF · " if blitz else "🟢 <b>KAUF · ") + self._titel(symbol) + "</b>",
            "",
            t.menge(shares) + " Stück zu " + t.preis(price) + " $"
            + "  =  " + t.de(einsatz) + " $",
            "Risiko: " + t.de(risiko) + " $ (" + t.de(risiko_pct, 1) + " % vom Konto)",
            "",
        ]
        if blitz:
            zeilen += [
                "Auslöser: Handelsvolumen " + t.de(spike_ratio, 1) + "× so hoch wie normal",
                "Enger Stop bei −1,5 %, Ziel +3 % — bewusst kurze Leine",
                "Blitz-Käufe heute: " + str(self._spike_count) + " von "
                + str(self.spike_max_day),
            ]
        else:
            zeilen += [
                "Lage: " + t.lage(regime) + ", "
                + ("nicht überkauft" if rsi < 70 else "bereits weit gelaufen"),
                "Signal: " + str(round(score_pct * 100)) + " % der Anzeichen sprechen dafür",
            ]
            if size_mult < 1.0:
                zeilen.append("Kleiner gekauft als üblich — "
                              + t.ZONE_DE.get(dd_zone, t.esc(dd_zone)) + ", "
                              + t.VIX_DE.get(vol_regime, t.esc(vol_regime)))
        zeilen += ["", "Kontostand: " + t.de(self.balance) + " $"]
        return "\n".join(zeilen)
        ...
        if len(self._btc_5min_window) >= 3:
            ...
            if drop_pct >= 2.0 and not self._btc_crash_mode:
                ...
                print("[BTC-GUARD] BTC -{:.1f}% in 5min — Crash-Stop auf {} Positionen".format(
                      drop_pct, n))
                if n > 0:
                    self.send("🚨 <b>Bitcoin stürzt ab</b> · −{:.1f} % in 5 Minuten"
                              "\n\nBitcoin zieht den ganzen Markt mit. Deshalb:"
                              "\n• Bei allen {} offenen Positionen wurde der Stop enger"
                              " gezogen (1,5 % unter dem Hoch)"
                              "\n• Es wird nichts Neues gekauft, bis sich BTC erholt"
                              .format(drop_pct, n), roh=True)
            ...
            try:
                ...
                for tx in txs:
                    ...
                    if (from_type == "exchange"
                        ...
                        if ok:
                            ...
                            self.send("🐋 <b>KAUF nach Großabfluss · " + self._titel(symbol) + "</b>"
                                      "\n\n" + tg_texte.menge(shares) + " Stück zu "
                                      + tg_texte.preis(price) + " $"
                                      "\n\nAuslöser: " + tg_texte.de(usd_m) + " Mio. $ haben die"
                                      " Börse " + tg_texte.esc(owner) + " verlassen — jemand"
                                      " Großes sammelt ein, statt zu verkaufen."
                                      "\nStop bei −" + tg_texte.de(WHALE_SL, 1) + " %, Ziel +"
                                      + tg_texte.de(WHALE_TP, 1) + " %"
                                      "\n\nKontostand: " + tg_texte.de(self.balance) + " $",
                                      roh=True)
                    ...
                    # ── ALERT trigger: massive inflow to exchange (distribution) ──
                        ...
                        msg = ("🐋 <b>Verkaufsdruck · " + self._titel(symbol) + "</b>"
                               "\n\n" + tg_texte.de(usd_m) + " Mio. $ sind zur Börse "
                               + tg_texte.esc(owner) + " geflossen — jemand Großes bringt"
                               " Coins zum Verkauf.\n\n")
                        if has_pos:
                            msg += ("Du hältst diese Position: der Stop wurde auf 0,5 %"
                                    " nachgezogen, damit wenig verloren geht.")
                            ...
                            msg += "Du hältst diese Position nicht — es wird auch nicht gekauft."
                        self.send(msg, roh=True)
    ...
    def _ws_spike_check(self, symbol, price):
        ...
        print(msg)
        self.send(self._tg_kauf(
            symbol, shares, price, None, 0, 0,
            shares * price * 0.015,          # Blitz-Kauf: Risiko = enger 1,5-%-Stop
            1.0, None, None, spike_ratio=ratio), roh=True)
            ...
            try:
                ...
                if outflow >= min_tokens:
                    ...
                    if outflow >= min_tokens * 5:
                        self.send("🐋 <b>Großer Abfluss · " + tg_texte.esc(ticker) + "</b>"
                                  "\n\n" + tg_texte.de(outflow) + " Token wurden von der Börse"
                                  " abgezogen — " + tg_texte.de(factor, 1) + "× so viel wie üblich."
                                  "\n\nWer Coins von der Börse holt, will sie halten, nicht"
                                  " verkaufen. Der Bot wertet das als Kaufargument."
                                  "\n<i>Hinweis, keine Order — gekauft wird nur, wenn auch"
                                  " die übrigen Bedingungen passen.</i>", roh=True)
                ...
                for name, bal_now in btc_now.items():
                    ...
                    if abs(delta) > 50:
                        ...
                        if delta > self._BTC_ALERT_LARGE:
                            scores["BTC/USD"] += 3.0
                            self.send("🐋 <b>Großer Abfluss · Bitcoin</b>"
                                      "\n\n" + tg_texte.vz(delta) + " BTC haben "
                                      + tg_texte.esc(name) + " innerhalb einer Stunde verlassen."
                                      "\n\nSolche Mengen wandern in die Verwahrung, nicht in den"
                                      " Verkauf. Der Bot wertet das als Kaufargument."
                                      "\n<i>Hinweis, keine Order.</i>", roh=True)
            ...
            if eth_now:
                ...
                if prev_eth and (now - prev_time) >= 3500:
                    ...
                    if total_delta > self._ETH_ALERT_LARGE:
                        scores["ETH/USD"] += 3.0
                        self.send("🐋 <b>Großer Abfluss · Ethereum</b>"
                                  "\n\n" + tg_texte.vz(total_delta) + " ETH haben die Börsen"
                                  " innerhalb einer Stunde verlassen."
                                  "\n\nDer Bot wertet das als Kaufargument."
                                  "\n<i>Hinweis, keine Order.</i>", roh=True)
        ...
        for symbol in symbols:
            try:
                ...
                if reason:
                    ...
                    # Keine eigene Meldung: close_position schickt gleich die
                    # Verkaufsnachricht inkl. Grund und Haltedauer. Vorher kamen
                    # zwei Nachrichten fuer denselben Vorgang.
    ...
    def close_position(self, symbol, price, reason, pnl_pct):
        ...
        print(msg)
        self.send(tg_texte.verkauf(self._titel(symbol), profit, pnl_pct, reason,
                                   pos.get("time", ""), self.balance), roh=True)
    ...
    def _get_drawdown_mult(self):
        ...
        if day_pct > -3.0:
            ...
            if not getattr(self, "_dd_caution_sent", False):
                self.send("⚠️ <b>Vorsicht</b> · " + tg_texte.vz(day_pct, 1) + " % heute"
                          "\n\nNeue Käufe ab jetzt 30 % kleiner."
                          "\nGehaltene Positionen bleiben unberührt.", roh=True)
                self._dd_caution_sent = True
            return 0.7, "CAUTION", True
        elif day_pct > -9.0:
            if not getattr(self, "_dd_warning_sent", False):
                self.send("🔴 <b>Warnung</b> · " + tg_texte.vz(day_pct, 1) + " % heute"
                          "\n\nNeue Käufe nur noch in 40 % der normalen Größe."
                          "\n<b>Meme-Coins werden gar nicht mehr gekauft.</b>", roh=True)
                self._dd_warning_sent = True
            return 0.4, "WARNING", False
        else:
            if not getattr(self, "_dd_danger_sent", False):
                self.send("🚨 <b>Gefahrenzone</b> · " + tg_texte.vz(day_pct, 1) + " % heute"
                          "\n\n<b>Es wird nichts mehr gekauft.</b>"
                          "\nGehaltene Positionen laufen mit ihren Stops weiter.", roh=True)
    ...
    def trade(self, scores):
        ...
        for symbol, score in ranked:
            ...
            print(msg)
            self.send(self._tg_kauf(
                symbol, shares, price, regime, ind.get("rsi", 0), score_pct,
                shares * ind["atr"] * 2, size_mult, dd_zone, vol_regime), roh=True)
        ...
        try:
            ...
            if os.path.exists(ctrl_path):
                ...
                if ctrl.get("command") == "stop":
                    ...
                    if syms:
                        self.send("🛑 <b>Risikobremse</b>"
                                  "\n\n" + str(len(syms)) + " Position(en) werden jetzt"
                                  " geschlossen, danach hält der Bot an."
                                  "\nJeder Verkauf kommt einzeln.", roh=True)
                    for sym in syms:
    ...
    def check_day_loss(self):
        ...
        if loss >= self.max_day_loss:
            ...
            print("[DAY_LOSS] " + msg)
            self.send("🛑 <b>Tagesverlust erreicht</b>"
                      "\n\nVerlust: <b>" + tg_texte.vz(-loss * 100, 1) + " %</b>"
                      "\nKontostand: " + tg_texte.de(balance) + " $"
                      "\n\n<b>Der Bot handelt weiter</b> — Krypto läuft rund um die Uhr,"
                      " eine Pause bis Mitternacht würde nur Chancen kosten."
                      "\nDie Grenze zählt ab jetzt neu ab diesem Stand.", roh=True)
    ...
    def run(self):
        ...
        self.send("▶️ <b>Crypto-Bot gestartet</b>"
                  ...
                  + "\nBetriebsart: " + ("Papierhandel" if self.demo else "ECHTES GELD"),
                  roh=True)
